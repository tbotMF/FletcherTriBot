...
public class MFFletcherGUI extends AbstractGUI {
	...
	public List<State> createStateAssociations() {
		...
		if (makeRadioButton.isSelected()) {
			making.addActions(new UseItemOneWithTwo(), new Make10Sets(), new MakeUnf());
			states.add(making);
		} else if (fletchRadioButton.isSelected()) {
			cutting.addActions(new UseKnife(),new SelectMakeX(), new EnterAmountToCut(), new Cut());
			states.add(cutting);
		} else {
			stringing.addActions(new UseBowString(), new SelectMakeAll(),new StringBow());
